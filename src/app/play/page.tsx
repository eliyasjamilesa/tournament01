...
import { 
  Users, Trophy, Search, Map as MapIcon, Skull, Wallet, Gamepad2, Calendar, Layers, Monitor, ArrowLeft, Globe, Lock, Loader2, Key, CheckCircle2, AlertCircle
} from 'lucide-react';
...
function RoomDetailsSheet({ tournament }: { tournament: any }) {
  ...
  const { data: registration, loading } = useDoc<any>(regDocRef);
...
export default function PlayPage() {
  ...
  const handleJoin = async (tournament: any) => {
    if (!db || !user) { toast({ title: "Login Required", description: "Please login to join matches." }); return; }
    if (tournament.currentPlayers >= tournament.maxPlayers) { toast({ variant: "destructive", title: "Match Full", description: "No more slots available." }); return; }
  ...
  const filteredTournaments = tournaments?.filter(t => t.title?.toLowerCase().includes(searchQuery.toLowerCase()) || t.matchId?.toLowerCase().includes(searchQuery.toLowerCase()));
    ...
    <div className="min-h-screen bg-background pb-32">
      ...
      <main className="p-6 space-y-6">
        <div className="relative">
          ...
          <Input placeholder="Search by ID or Title..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-11 bg-muted/30 border-white/5 h-11 rounded-xl text-sm" />
        </div>

        <div className="space-y-6">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-3"><Loader2 className="w-8 h-8 animate-spin text-primary opacity-50" /><p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Syncing Arenas...</p></div>
          ) : filteredTournaments?.length === 0 ? (
            <div className="text-center py-20 bg-muted/10 rounded-3xl border border-white/5 border-dashed"><p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">No Battlefields Detected</p></div>
          ) : filteredTournaments?.map((t) => (
            <Card key={t.id} className="border-white/5 bg-card overflow-hidden rounded-2xl shadow-xl">
              <div className="p-5 space-y-5">
                <div className="flex items-start justify-between">
                  <div className="flex gap-3">
                    <div className="w-12 h-12 rounded-xl bg-primary flex items-center justify-center"><Gamepad2 className="text-white w-6 h-6" /></div>
                    <div>
                      <h3 className="text-sm font-black uppercase italic tracking-tight leading-none mb-1">{t.title} <span className="text-primary ml-1">{t.mode}</span></h3>
                      <div className="flex items-center gap-1.5 text-[9px] font-bold text-green-500 uppercase"><Calendar className="w-3 h-3" />{t.startTime ? format(new Date(t.startTime), 'dd MMM, hh:mm a') : 'TBA'}</div>
                    </div>
                  </div>
                  <div className="bg-primary text-white font-black text-[10px] py-1 px-3 rounded-lg">{t.matchId || '#----'}</div>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  {[ { label: 'FEE', val: `${t.entryFee} TK`, icon: Wallet }, { label: 'PRIZE', val: `${t.prizePool} TK`, icon: Trophy }, { label: 'KILL', val: `${t.perKill} TK`, icon: Skull }, { label: 'MAP', val: t.map, icon: MapIcon }, { label: 'VER', val: t.version, icon: Monitor }, { label: 'TYPE', val: t.mode, icon: Layers } ].map((stat, i) => (
                    <div key={i} className="bg-muted/30 rounded-lg p-2 flex flex-col items-center justify-center border border-white/5"><span className="text-[7px] font-bold uppercase text-muted-foreground mb-0.5">{stat.label}</span><span className="text-[10px] font-black truncate w-full text-center">{stat.val}</span></div>
                  ))}
                </div>
                <div className="space-y-2">
                  <div className="flex justify-between items-center"><span className="text-[9px] font-bold uppercase text-muted-foreground">Registered Players</span><span className="text-[10px] font-black">{t.currentPlayers}/{t.maxPlayers}</span></div>
                  <div className="h-1.5 w-full bg-muted rounded-full overflow-hidden"><div className="h-full bg-primary transition-all duration-500" style={{ width: `${Math.min(100, (t.currentPlayers / t.maxPlayers) * 100)}%` }} /></div>
                </div>
                <div className="flex flex-col gap-2">
                  <Button onClick={() => handleJoin(t)} disabled={t.currentPlayers >= t.maxPlayers || joining === t.id} className="w-full h-11 rounded-xl font-black uppercase italic tracking-wider text-xs">
                    {joining === t.id ? <Loader2 className="w-4 h-4 animate-spin" /> : t.currentPlayers >= t.maxPlayers ? <><Lock className="w-3.5 h-3.5 mr-2" /> Match Full</> : 'Join Match'}
                  </Button>
