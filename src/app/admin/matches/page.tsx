    ...
    <div className="space-y-8 pb-20">
      <Tabs defaultValue={initialTab} className="space-y-6">
        <TabsList className="bg-muted/30 h-11 p-1 rounded-xl border border-white/5 w-full">
          <TabsTrigger value="manage" className="flex-1 rounded-lg font-black text-[10px] uppercase">Results</TabsTrigger>
          <TabsTrigger value="deploy" className="flex-1 rounded-lg font-black text-[10px] uppercase">Upload</TabsTrigger>
        </TabsList>

        <TabsContent value="deploy" className="animate-in fade-in zoom-in-95 duration-300">
          <Card className="border-white/5 bg-card/40 rounded-3xl overflow-hidden">
            <CardContent className="pt-6">
              <form onSubmit={handleAddMatch} className="space-y-6">
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    <Label className="text-[10px] font-bold uppercase ml-1">Title</Label>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div className="space-y-1.5"><Label className="text-[10px] font-bold uppercase ml-1">Fee</Label><Input type="number" value={entryFee} onChange={(e) => setEntryFee(e.target.value)} className="bg-muted/50 border-white/5 h-12 rounded-xl" required /></div>
                    ...
                    <div className="space-y-1.5"><Label className="text-[10px] font-bold uppercase ml-1">Kill</Label><Input type="number" value={perKill} onChange={(e) => setPerKill(e.target.value)} className="bg-muted/50 border-white/5 h-12 rounded-xl" required /></div>
                  </div>
                </div>
        ...
        <TabsContent value="manage" className="animate-in fade-in zoom-in-95 duration-300">
          <div className="space-y-4">
            {tournamentsLoading ? (
              ...
              <Card key={match.id} className="border-white/5 bg-card/40 p-5 rounded-2xl overflow-hidden">
                <div className="flex items-center justify-between mb-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-xs font-black uppercase italic text-white">{match.title}</h4>
                  ...
                  <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-red-500" onClick={() => {
                    if(confirm('Delete this arena?')) deleteDoc(doc(db, 'tournaments', match.id));
                ...
                <div className="flex gap-2">
                  <Dialog>
                    <DialogTrigger asChild>
                      <Button variant="outline" size="sm" onClick={() => setEditingRoom({id: match.id, rid: match.roomId || '', rpass: match.roomPassword || ''})} className="flex-1 h-10 rounded-xl text-[9px] font-black uppercase border-white/5 bg-white/5"><Key className="w-3 h-3 mr-2 text-primary" /> Room Info</Button>
                    </DialogTrigger>
                    <DialogContent className="bg-card border-white/5 rounded-3xl max-w-[340px]">
                      ...
                      <div className="space-y-4 py-4">
                        <div className="space-y-1.5"><Label className="text-[10px] font-bold uppercase">Room ID</Label><Input value={editingRoom?.rid} onChange={(e) => setEditingRoom(prev => prev ? {...prev, rid: e.target.value} : null)} className="bg-muted/50 border-none h-11 rounded-xl" /></div>
                        <div className="space-y-1.5"><Label className="text-[10px] font-bold uppercase">Password</Label><Input value={editingRoom?.rpass} onChange={(e) => setEditingRoom(prev => prev ? {...prev, rpass: e.target.value} : null)} className="bg-muted/50 border-none h-11 rounded-xl" /></div>
                        <Button onClick={handleUpdateRoom} className="w-full magma-gradient h-12 font-black uppercase italic rounded-xl">Save</Button>
                      </div>
                    </DialogContent>
                  </Dialog>
                  ...
                  <Dialog>
                    <DialogTrigger asChild>
                      <Button variant="outline" size="sm" onClick={() => fetchRegistrations(match.id)} className="flex-1 h-10 rounded-xl text-[9px] font-black uppercase border-white/5 bg-white/5"><Medal className="w-3 h-3 mr-2 text-yellow-500" /> Results</Button>
                    </DialogTrigger>
                    <DialogContent className="bg-card border-white/5 rounded-3xl max-h-[80vh] overflow-y-auto max-w-[340px]">
                      <DialogHeader><DialogTitle className="text-sm uppercase font-black italic">Winner Stats</DialogTitle></DialogHeader>
                      <div className="space-y-3 py-4">
                        {registrations.length === 0 ? (
                           <p className="text-center text-[10px] text-muted-foreground uppercase py-10">No registrations</p>
                        ) : registrations.map((reg) => (
                          <div key={reg.id} className="p-3 bg-muted/30 rounded-xl flex items-center justify-between border border-white/5">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="text-[9px] font-black uppercase">{reg.ingameName || reg.displayName}</p>
                                <p className="text-[7px] font-bold text-muted-foreground uppercase">Slot #{reg.slotNumber}</p>
                              </div>
                            </div>
                            <div className="flex items-center gap-2">
                              <span className="text-[8px] font-black text-muted-foreground">TK</span>
                              <Input type="number" placeholder="0" value={reg.wonAmount || ''} onChange={(e) => setRegistrations(prev => prev.map(r => r.id === reg.id ? { ...r, wonAmount: e.target.value } : r))} className="w-14 h-7 text-[10px] font-black bg-background border-none text-right" />
                            </div>
                          </div>
                        ))}
                        {registrations.length > 0 && (
                          ...
                          }} className="w-full magma-gradient h-12 font-black uppercase italic rounded-xl mt-4">Save All</Button>
                        )}
                      </div>
                    </DialogContent>
                  </Dialog>
