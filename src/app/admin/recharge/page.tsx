...
export default function RechargePage() {
  ...
  const handleRecharge = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Sent Successfully", description: `${amount} TK added.` });
      setEmail(''); setAmount('');
    } catch (err: any) {
  ...
  return (
    <div className="space-y-8 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="space-y-1">
        <h2 className="text-2xl font-black uppercase italic tracking-tighter text-white">Payment <span className="text-primary">Send</span></h2>
        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Manual coin recharge</p>
      </div>
        ...
        <CardContent className="p-6 space-y-6">
          <form onSubmit={handleRecharge} className="space-y-6">
            ...
            <Button type="submit" disabled={isProcessing} className="w-full h-14 magma-gradient font-black uppercase italic rounded-xl shadow-lg">
              {isProcessing ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Send Coins'}
            </Button>
          </form>
          ...
          <div className="p-4 rounded-xl bg-white/5 border border-white/5 flex items-start gap-3">
             ...
             <p className="text-[8px] font-bold text-muted-foreground uppercase leading-relaxed">Administrator, this action will immediately update the balance.</p>
          </div>
        </CardContent>
      </Card>
    </div>
